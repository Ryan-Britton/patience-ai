...
llm = ChatOpenAI(temperature=0, model_name=os.getenv("OPENAI_MODEL", "gpt-4o-mini"))
...
classify_tool = StructuredTool.from_function(classify_email, name="classify_email",
    description="Classify incoming email by persona and type")

def noop_tool(data, cfg=None):
    return data

update_categories_tool = StructuredTool.from_function(noop_tool, name="update_categories",
    description="Stub – calls n8n webhook to patch Outlook categories")

memory_tool = StructuredTool.from_function(noop_tool, name="write_memory",
